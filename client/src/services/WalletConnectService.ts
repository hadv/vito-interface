...
export class WalletConnectService {
  ...
  private patchWalletConnectInternalValidation(): void {
    ...
    try {
      ...
      if ((this.signClient as any).session?.store?.get) {
        ...
        (this.signClient as any).session.store.get = (topic: string) => {
          try {
            ...
            console.warn('🛡️ WalletConnect session store get failed, returning null:', topic);
            return null;
          }
        };
      }
      ...
      if ((this.signClient as any).session?.store?.getData) {
        ...
        (this.signClient as any).session.store.getData = (topic: string) => {
          try {
            ...
            console.warn('🛡️ WalletConnect session store getData failed, throwing controlled error:', topic);
      ...
      console.log('✅ WalletConnect internal validation methods patched successfully');
    } catch (error) {
      console.warn('⚠️ Failed to patch WalletConnect internal validation methods:', error);
    }
  }
