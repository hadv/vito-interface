...
const TransactionModal: React.FC<TransactionModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Propose transaction only (without signing)
      const result = await errorRecoveryService.retry(async () => {
        ...
        if (preSelectedAsset && preSelectedAsset.type === 'erc20' && preSelectedAsset.contractAddress) {
          ...
          return await safeWalletService.proposeTransactionOnly({
      ...
      console.log('📋 Result from proposeTransactionOnly:', result);

      setSuccess(`Transaction proposed successfully! Hash: ${result.txHash}`);

      toast.transactionSuccess(result.txHash, 'Transaction proposed to SafeTxPool');
      ...
      console.error('❌ TRANSACTION MODAL: Error proposing transaction:', error);
