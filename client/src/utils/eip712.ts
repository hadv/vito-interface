...
export function combineSignatures(signatures: Array<{ signature: string; signer: string }>): string {
  ...
  for (const { signature, signer } of sortedSignatures) {
    ...
    console.log(`🔐 Processing signature from ${signer}:`, {
      original: signature,
    ...
    if (adjustedSignature.length === 132) { // 0x + 64 + 64 + 2 = 132 chars
      ...
      if (v === 27) {
        ...
        console.log(`🔐 Adjusted v: 27 -> 31`);
      } else if (v === 28) {
        adjustedSignature = adjustedSignature.slice(0, -2) + '20'; // 28 -> 32
        console.log(`🔐 Adjusted v: 28 -> 32`);
      } else {
        console.log(`🔐 V value ${v} not adjusted`);
      }
    } else {
      console.warn(`🔐 Unexpected signature length: ${adjustedSignature.length}`);
    }

    console.log(`🔐 Final signature for ${signer}:`, adjustedSignature);
